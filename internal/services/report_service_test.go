...
func (f *fakeArticleRepo) Create(article *dbmodels.Article) error {
	f.items = append(f.items, article)
	return nil
}
func (f *fakeArticleRepo) CreateForReport(reportID string, article *dbmodels.Article) error {
	return f.Create(article)
}
...
func (f *fakeArticleRepo) List(limit, offset int) ([]*dbmodels.Article, int64, error) {
	return nil, 0, nil
}
...
func (f *fakeArticleRepo) GetGRCArticles(limit, offset int) ([]*dbmodels.Article, int64, error) {
	return nil, 0, nil
}
