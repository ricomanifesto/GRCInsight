...
| `editorial-corrections.json` | Original and corrected report/manifest hashes for disclosed editorial exclusions. |
...
Tagged event promotions are excluded as whole source records before analysis and are rejected at publication. They are never relabeled as news.

A retained report may receive an explicit editorial exclusion without rerunning its authoring model. The report discloses the correction, preserves its original generation metadata and analyzed-article counts, and lists only retained public evidence. Its manifest identifies the correction record. That record binds original and corrected Markdown and manifest hashes; the original publication journal is not rewritten. Missing correction records or changed corrected bytes fail validation. Only these hash-bound corrections permit rebuilding an existing archive body.
