"""OpenCode HTTP client for provider-switchable model calls."""
...
class ModelSelection:
    """Provider and model IDs accepted by OpenCode."""
...
def parse_model_selection(model_name: str) -> ModelSelection:
...
class OpenCodeClient:
    """Small async client for OpenCode's session message API."""
    ...
    def __init__(
        ...
        self.base_url = (
            base_url or os.getenv(OPENCODE_BASE_URL_ENV_VAR, DEFAULT_OPENCODE_BASE_URL)
        ).rstrip("/")
    ...
    def _raise_for_status(self, response: httpx.Response, action: str) -> None:
        if response.is_success:
            return
        raise OpenCodeError(f"Failed to {action}: HTTP {response.status_code} {response.text}")
