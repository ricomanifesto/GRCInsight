...
def _build_fallback_report(
    ...
    """Create a deterministic Markdown report when OpenAI is unavailable."""
    ...
    if not risk_lines:
        risk_lines.append("- Current source articles emphasize operational cyber risk and compliance monitoring.")
    ...
    if regulations or frameworks or bodies:
        ...
        regulatory_lines.append("- No explicit regulation codes or named frameworks were detected in the current source set.")
        regulatory_lines.append("- Governance and compliance significance is inferred from enforcement, breach, and risk-management reporting patterns.")

    industry_lines = []
    if industries:
        industry_lines.append(f"- Industries with explicit mentions: {', '.join(industries[:6])}.")
    else:
        industry_lines.append("- The current feed is broad-based cybersecurity reporting with cross-sector relevance for governance, risk, and compliance teams.")

    limitation = fallback_reason or "OpenAI analysis unavailable"
    ...
    return "\n".join([
        ...
        "",
        "1) Executive Summary",
        f"- This report was generated using deterministic local analysis because OpenAI generation was unavailable: {limitation}.",
...
async def run_grc_analysis_endpoint(feed_url: str, config: AnalysisConfig) -> WorkflowResponse:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"OpenAI service unavailable, falling back to local analysis: {e}")
        ...
        if openai_service is not None:
            ...
            if "error" in analysis_results:
                fallback_reason = analysis_results["error"]
                analysis_results = local_analysis
                logger.warning(f"GRC analysis failed, using local fallback analysis: {fallback_reason}")
            else:
                used_openai_analysis = True
        ...
        report_content = ""
        if used_openai_analysis and openai_service is not None:
            ...
            if not report_content or report_content.startswith("# GRC Intelligence Report - Error"):
                fallback_reason = report_content or "empty report content"
                report_content = ""
        ...
        if not report_content:
            ...
            logger.warning("Generated fallback report because OpenAI output was unavailable")
